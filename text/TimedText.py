...
class TimedText:
    ...
    def populate_timed_text_parameters(
        self,
        text: str,
        font,
        font_size: int,
        ...
        self.upper_threshold = (
            ClockMethods.get_current_time_in_seconds() + time_interval
        )
    ...
    def run(self, screen) -> None:
        ...
        if not ClockMethods.is_past_this_time_in_seconds(self.upper_threshold):
            Text.render(
                screen, self.text, self.font, self.font_size, self.color, self.location
            )
