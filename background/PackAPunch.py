...
class PackAPunch:
    ...
    def __draw_pap_box(self, screen, player: Player, cost: int, trying_to_buy: bool) -> None:
        """
        Draws PaP box
        """
        ...
        if player.x >= self.sparkles_threshold:
            ...
            Text.render(screen, "Press B to Upgrade Weapon", Text.Font.Euro_Horror, 18, Colors.Neon_Orange, (self.x_start - 70, self.y_start - 55))
            Text.render(screen, f"Costs {cost} points", Text.Font.Euro_Horror, 18, Colors.Neon_Orange, (self.x_start - 10, self.y_start - 30))
            ...
            if trying_to_buy \
                ...
                self.timed_text.populate_timed_text_parameters("Cannot upgrade an upgraded weapon",
                                                                    Text.Font.Euro_Horror, 20, Colors.Red,
                                                                    (210, 130), 3)
            ...
            if trying_to_buy \
                ...
                if self.__is_inbounds(player):
                    ...
                    self.mystery_weapon = PackAPunch.pack_a_punch(player.get_current_weapon())
    ...
    def __draw_weapon(self, screen, player: Player, trying_to_pick_up_weapon: bool, weapon: Weapon) -> None:
        ...
        if player.x >= self.sparkles_threshold:
            ...
            Text.render(screen, f"Press C for {weapon.name}", Text.Font.Euro_Horror, 18, Colors.Neon_Orange, (self.x_start - weapon.image_width + 10, self.y_start - 35))
